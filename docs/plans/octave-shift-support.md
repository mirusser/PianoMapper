...
`type="down"` is the 8va-alta case, where the notation sits below the performed pitch. `type="up"` is the 8va-bassa case, where the notation sits above the performed pitch. The note's MusicXML `<pitch>` contains the performed/sounding pitch. The reader must preserve that pitch and use the signed octave-shift value only to reconstruct the written staff position. An earlier implementation treated `<pitch>` as written pitch and transposed it a second time; the standards-compliant fixtures and tests now guard against that regression.
...
- A MusicXML file containing `<direction><direction-type><octave-shift type="down" size="8" number="1"/></direction-type></direction>` ... notes ... `<direction><direction-type><octave-shift type="stop" size="8" number="1"/></direction-type></direction>` imports without error; the enclosed notes' `Pitch` preserves the sounding MusicXML `<pitch>` unchanged, and their `SoundingOctavesAboveNotated` is `1`.
- The mirror-image case (`type="up"`, the 8va-bassa/"sounds lower" direction) preserves the sounding MusicXML `<pitch>` and produces `SoundingOctavesAboveNotated` of `-1`, proving the direction mapping is not backwards.
...
### Task 3: Attach active-shift metadata without transposing notes; the real 8va/8vb fixture

**Description:** Thread `activeOctaveShiftOctaves` (by value, not `ref` — only the `"direction"` case mutates it) into `ParseNote`. Preserve the sounding pitch read from MusicXML `<pitch>` unchanged, and set `SoundingOctavesAboveNotated: activeOctaveShiftOctaves` on the constructed `ScoreNote`. Rendering subtracts that offset to recover the written staff position. Add `PianoMapper.Tests/Fixtures/octave-shift-8va.musicxml`: one measure, an unshifted note, then `<octave-shift type="down" size="8" number="1"/>`, three or four shifted notes whose `<pitch>` values are already the higher performed pitches, `<octave-shift type="stop" size="8" number="1"/>`, then an unshifted closing note. Add a second fixture for the mirror-image `type="up"` (8va-bassa) direction whose `<pitch>` values are already the lower performed pitches, proving the sign is not backwards.

**Acceptance criteria:**
- [x] `octave-shift-8va.musicxml`: notes before the shift have `SoundingOctavesAboveNotated == 0`; notes inside the shift preserve their higher, sounding MusicXML `<pitch>` and have `SoundingOctavesAboveNotated == 1`; notes after the stop revert to offset `0`.
- [x] The mirror `type="up"` fixture: shifted notes preserve their lower, sounding MusicXML `<pitch>` and have `SoundingOctavesAboveNotated == -1`.
- [x] `size="15"`/`size="22"` cases (heredoc, one each) produce `SoundingOctavesAboveNotated` of `±2`/`±3`.
- [x] New end-to-end tests prove a shifted note's preserved `Pitch.MidiNumber` is what `Grader.Classify`/`ScoreDerivation.Flatten` grades as correct and that browser playback schedules the exact frequency of that sounding pitch. Grading and audio logic need zero changes.
