# Browser Test Matrix

Test the published PWA over HTTPS. Browser extensions, power-saving modes, and audio-device routing can change the result, so record the browser version and device with each manual run.

## Current evidence

| Browser | Platform | MIDI, shortcuts, and audio | MusicXML, playback, practice | Resize and offline | Status |
|---|---|---|---|---|---|
| Headless Chromium 149 | Linux (`archie`) | Prior audio unlock, polyphony, analyser, and shortcut pass; Web MIDI rerun pending | Supported and malformed import, score schedule, paired-row navigation/playback/Practice, practice start/abort/retry, random measure | Canvas batches, paired-row responsive widths, unload/reload, view toggling, and published offline reload | Prior non-MIDI pass, 2026-08-02; rerun required |
| Chrome stable | Primary laptop | Pending physical FP-10 audio and latency check | Pending | Pending install check | Manual pass required |
| Edge stable | Desktop | Pending | Pending | Pending install check | Manual pass required |
| Firefox 153.0.1 | Linux (`archie`) | FP-10 enumeration, permission flow, piano-sample audio, and a physical C8 note-on passed; full endpoint/shortcut pass pending | Pending | Pending offline check | Physical MIDI smoke pass, 2026-09-11 |
| Safari stable | macOS | No test device selected | No test device selected | No test device selected | Access decision required |

## Manual pass

Use one supported MusicXML fixture and one malformed file.

1. Connect and power on the MIDI piano, load the page over HTTPS or localhost, and grant MIDI permission. In Firefox, select **Remember this decision** to enable automatic reconnection on later visits. Confirm the Audio panel names both the MIDI input and output and Enable audio can recover after a blocked or failed attempt.
2. Play the A0 and C8 endpoints, then hold a chord and release each piano key. Switch views with V, resize the window, and confirm pitch, velocity response, highlighting, and release behavior without stuck notes.
3. Confirm `A W S E D F R J U K I L ;`, `Z`/`X`, and `1`–`8` do not play or remap notes. Use Tab, Enter, Space, Escape, Page Up/Down, and the arrow keys for their browser-native behavior. Type in the file input and notation-focus selector without triggering piano commands.
4. Load the supported score. Use the buttons and `[`/`]` to navigate. Enable **Edit fingering**, select notes by canvas and by the note list, assign and remove fingerings, then use **Regenerate all fingerings** and confirm that every number is replaced while hand assignments remain unchanged. Play and restart the score with P.
5. Start practice with T, play it from the MIDI piano, abort with C, retry, and hide the tab during a second run. The hidden run must abort and release audio.
6. Play a random measure with M. Return to the loaded score and play it again.
7. Close the tab after the online load, go offline, reopen the installed or cached PWA, and confirm the shell starts. MusicXML fixtures must still come from user file selection.

Record console errors, audible glitches, stuck notes, focus loss, and controls without a visible focus ring as failures.

## Imported-score paired grand staff

Use a score with at least 16 measures, preferably the local `Humpty-Dumpty.mxl` fixture when available. Also select a range of five measures or fewer and a range whose last page is partial. Do not add local MusicXML fixtures to source control.

| Scenario | Action | Expected result | Result |
|---|---|---|---|
| Initial preload | Load the long score while the grand staff is visible. | The upper row shows measures 1–5 and is labeled Current. The lower row shows measures 6–10 and is labeled Look ahead. | Pass — Chromium 149, 2026-08-02 |
| Upper-to-lower handoff | Start playback or Practice and cross the measure 5/6 boundary. | The cursor appears on the lower row only. The upper row is then recycled to measures 11–15 without an audible restart. | Visual/continuity pass — Chromium 149, 2026-08-02; physical audio pending |
| Lower-to-upper handoff | Continue across the measure 10/11 boundary. | The cursor appears on the preloaded upper row only. The lower row is then recycled to measures 16–20, or the available partial page. | Pass — Chromium 149, 2026-08-02 |
| Final partial page | Let playback or Practice finish on a partial final page. | The final page stays visible and labeled Current; the immediately previous page remains in the inactive row. | Pass — Chromium 149, 2026-08-02 |
| Short selected range | Select five measures or fewer. | One labeled grand-staff row is visible. No empty or duplicate second row appears. | Pass — Chromium 149, 2026-08-02 |
| Manual navigation | While idle, use Previous/Next and `[`/`]`. Repeat while playback and Practice are active. | Idle navigation advances one logical five-measure page and updates Current. Navigation is disabled and ignored while either following mode is active. | Pass — Chromium 149, 2026-08-02 |
| Restart and range reset | Browse away from the first page, then use Play / restart. Change the selected measure range. | Both actions return to the selected range's first current/look-ahead pair. | Pass — Chromium 149, 2026-08-02 |
| Practice overlays | During Practice, hold notes and cross two page boundaries. | Verdict colors persist after recycling. The cursor and held-note indicators appear only on their owning row. | Pass — Chromium 149, 2026-08-02 |
| View toggle | During playback and Practice, switch staff → piano roll → staff. Unload and reload the score afterward. | The paired rows return with the correct pages and cursors. The console shows no stale-element, interop, resize-observer, or disposal errors. | Pass — Chromium 149, 2026-08-02 |
| Responsive widths | Repeat initial load and both handoffs at 1440, 1024, 768, and 320 CSS pixels. | Clefs, signatures, notes, ledgers, ties, barlines, labels, and cursor remain legible without an internal score scroller. At 320 px, horizontal document scrolling preserves the score's minimum readable width. | Pass — Chromium 149, 2026-08-02 |
| Fingering selection | Enable **Edit fingering** and select notes in both rows by canvas and by the note list. Include two members of the same chord. | The selected note has a visible ring, the editor identifies its measure, beat, pitch, and hand, and each chord member selects independently. | Pending |
| Fingering edit | Assign 1–5 to notes with and without imported fingerings, switch selected notes between **Right hand** and **Left hand**, change an existing fingering, then remove one. | The grand-staff label and selected hand update immediately. Replacing an imported number retains its established placement behavior, and no sibling note changes. | Headless Chrome 149 right-to-left hand edit pass, 2026-09-14; number/removal manual checks pending |
| Fingering edit state | While editing, navigate pages and change the selected measure range and meter. Then start playback and Practice. | Completed edits remain. Navigation, range, or meter changes clear stale selection; playback and Practice leave edit mode and cannot modify notes. | Pending |
| Fingering generation | Set deliberately incorrect finger numbers on notes assigned to both hands, then select **Regenerate all fingerings**. Cancel once and confirm once. | Cancel leaves every number unchanged. Confirm replaces all finger numbers, keeps every hand assignment, displays all generated labels, and reports that **Save changes** is required for a saved score. | Confirmed action/status and no console errors pass — headless Firefox 153.0.1, 2026-09-14; cancel and visual hand checks pending |
| Fingering save round-trip | Edit an imported score and use **Save score**. Edit a loaded library score and use **Save changes**, then reload each entry. | Assigned hands plus assigned, changed, and removed fingerings match the saved state after reload. | Headless Chrome 149 number and hand save/update/reload pass, 2026-09-14; manual removal check pending |
| Saved-score deletion | Delete a non-current entry and cancel once, then confirm. Repeat for the currently open entry. | Cancel preserves the row. Confirm removes it from the list and database. Deleting the open entry keeps the score open and changes **Save changes** to **Save score**. | Headless Chrome 149 cancel, non-current, and current-entry pass, 2026-09-14 |

## Free Play measure-boundary ties

Run these checks on the live grand staff. Record the browser, viewport, sound source, time signature, and result for each row.

| Scenario | Action | Expected result | Result |
|---|---|---|---|
| Internal barline | In 4/4, press a note on the final beat of a measure and hold it into the next measure. | Two noteheads appear inside their measures with one tie between them. The sound continues without a second attack. | Pending |
| Five-measure rollover | Hold a note across the boundary after the fifth visible measure. | The next view starts with a continuation head and incoming tie stub inside the score area. No note element enters the clef or signature strip. | Pending |
| Exact boundary release | Release a note as the cursor reaches a barline. | The finished measure contains one notehead. No empty continuation head or extra tie appears. | Pending |
| Accidental | Hold a sharp or flat pitch across a barline. | The accidental appears at the attack and does not repeat on continuation heads. | Pending |
| Chord | Hold two or more pitches across a barline, then release their keys separately. | Each pitch receives its own fragments and ties. Each sound stops once on its key release. | Pending |
| Time signature | Repeat the internal and rollover checks in 3/4. | Fragment boundaries follow the 3-beat measures, and playback remains continuous. | Pending |
| Sound source | Repeat an internal crossing with piano, then synth. | Both sources sustain through the barline without a new attack. | Pending |
| Responsive layout | Repeat an internal crossing in a narrow viewport and a wide viewport. | Ties stay clipped to the score and keep the same staff-relative curve and stroke proportions. | Pending |
| Piano roll | Switch to the piano roll while holding a note across a grand-staff barline. | The performance still appears as one continuous piano-roll bar. | Pending |

## USB MIDI lifecycle

Use the Roland FP-10 through its USB COMPUTER port.

| Scenario | Action | Expected result | Result |
|---|---|---|---|
| Initial permission | Open the app on localhost or HTTPS and allow MIDI access. | The Audio panel reports `Roland Digital Piano`; playing a key starts audio and highlights the matching full-piano key. | Firefox 153.0.1 physical pass with C8, 2026-09-11 |
| Authorized reload | Reload the page after granting MIDI permission. | The existing Roland input reconnects without another in-page action. | Firefox 153.0.1 physical pass, 2026-09-11 |
| Velocity | Play the same key softly and firmly using the PC piano sound source. | Both attacks register and the selected sample layer follows the MIDI velocity. | Pending |
| FP-10 sound | Select **FP-10**, play physical keys, then use **Play test note** and score playback. | Physical keys sound once through the piano; generated notes use the FP-10's selected tone and speakers. The MIDI output summary names Roland rather than `Midi Through`. | Automated routing pass; physical audio pending, 2026-09-12 |
| Note-off forms | Play and release notes normally. | Explicit note-off and zero-velocity note-on messages both release without sticking. | Automated pass, 2026-09-11 |
| Hot disconnect | Hold a chord and unplug the USB cable. Reconnect it afterward. | Held app notes release, status changes to no input, and the reconnected piano is detected without a page reload. | Pending |
| Computer note keys removed | Press the former note and octave shortcuts. | No note sounds and the notation focus does not change; remaining command shortcuts continue to work. | Automated pass, 2026-09-11 |
