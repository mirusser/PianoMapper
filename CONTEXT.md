...
| **NoteValue** | A rhythmic value represented by a denominator (whole through sixteenth), optional dots, and an optional tuplet ratio (`actual`:`normal` notes, e.g. 3:2 for a triplet; 1:1 means no tuplet). | Tempo, wall-clock duration, onset, and pitch. Nested or nonuniform-ratio tuplet notation beyond the ratio itself. |
...
| **ScoreNote** | One expected score event with a sounding Pitch (preserved from MusicXML `<pitch>`), NoteValue, onset, Staff, tie-to-next flag, beam state, an optional sounding-vs-notated octave offset used only to recover written staff position, and optional imported stem direction, fingering, accidental, fermata, articulation (staccato/tenuto/accent), ornament (trill mark), accidental-mark, slur pairing, arpeggio mark, and glissando/slide pairing notation. Chord members share an onset. | Measured player timing, audio handles, rendering coordinates, computed fallback notation geometry, and `<other-notation>` content (accepted for import validity, never rendered). |
